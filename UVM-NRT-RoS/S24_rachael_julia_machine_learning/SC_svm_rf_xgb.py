...
def wav_to_Ht_Hf(folder_path):
    ...
    for filename in os.listdir(folder_path):
        ...
        #TODO: Try maad.sound.load ??

        #-------------Spectral Cover (SC)---------------------------------
        #Using the maad spectrogram for SC to get the ext variable,
        #to be able to get the spectrogram with no noise. TODO: May have others
        #use this as well?
...
# Random Forest without leave one out
X_train, X_test, Y_train, Y_test = train_test_split(X, Y, test_size=0.3, random_state=42)
clf = LogisticRegression(random_state=0) #TODO: Change random_state?
clf.fit(X_train, Y_train)
Y_pred = clf.predict(X_test)
accuracy_RF = accuracy_score(Y_test, Y_pred)
all_accuracies.append(accuracy_RF)
...
#-------------------------Plotting accuracies---------------------------------------------------
# Plotting accuracies
methods = ['SVM', 'SVM (w/o CV)', 'Random Forest', 'Random Forest (w/o CV)', 'Logistic Regression', 'Logistic Regression (w/o CV)', 'XGBoost', 'XGBoost (w/o CV)']
plt.figure(figsize=(10, 6))
bars = plt.bar(methods, all_accuracies, color=['blue', 'orange', 'yellow', 'pink', 'purple', 'green', 'brown', 'red'])
plt.title('Accuracy Comparison Spectral Cover (SC)')
plt.xlabel('Method')
plt.ylabel('Accuracy')
plt.ylim(0, 1)
...
plt.savefig('images/SC_SVM_RF_Graph.png')
plt.show()
