...
// MFCC Parameters
int numFilters = 10;        // Number of Mel filters
int numMFCCs = 14;          // Number of MFCC coefficients
float lowFreq = 50.0;       // Low-frequency cutoff in Hz
float highFreq = 12000.0;    // High-frequency cutoff in Hz
float mfccOutput[14];       // Array to store MFCCs for each frame

void calculateMFCC(
    int data[], size_t fftSize, float samplingFreq, size_t numFilters, size_t numMFCCs,
    float lowFreq, float highFreq, float mfccOutput[]);
...
void loop() {
    ...
    while (wavFile.available() && frameIndex < numFrames_overlap) {
        ...
        // Perform FFT (Assume Approx_FFT returns FFT magnitudes in `data`)
        float f = Approx_FFT(data, N, SAMPLING_FREQ);
        Serial.println("Dominant Frequency:");
        Serial.println(f);

        // Compute MFCCs within the FFT function - Go to Line 308 (or near this number)
...
FFT Code by ABHILASH and Soheyl F. Hagh
...
{ // Calculate amplitudes from the complex number
    out_r[i] = fastRSS(out_r[i], out_im[i]); // Compute the magnitude (approximate RSS)

    // Uncomment to Write the  magnitude to the SD card
    /*if (i == 1) {
...
audioFile.println(); // Start a new row for FFT magnitudes
  ...
  // ============================= Compute MFCCs ============================================
  calculateMFCC(out_r, N, SAMPLING_FREQ, numFilters, numMFCCs, lowFreq, highFreq, mfccOutput); 

  for (int i = 0; i < numMFCCs ; i++)      // Print MFCCs and Write onto SD card
  {
    if (i == 0) 
        audioFile.print(mfccOutput[i], 7); // Write the first magnitude without a leading comma
     else 
    {
        audioFile.print(",");      // Add a comma for subsequent magnitudes
        audioFile.print(mfccOutput[i], 7); // Write the magnitude
    }
  }
...
// MFCC Function
void calculateMFCC(
    int data[],           // Input data array (FFT magnitudes or power spectrum)
    size_t fftSize,       // FFT size
    float samplingFreq,   // Sampling frequency in Hz
    size_t numFilters,    // Number of Mel filters
    size_t numMFCCs,      // Number of MFCC coefficients
    float lowFreq,        // Low-frequency cutoff for Mel filterbank
    float highFreq,       // High-frequency cutoff for Mel filterbank
    float mfccOutput[])   // Output array for MFCCs
{
    size_t fftBins = fftSize / 2 + 1; // Number of FFT bins (half + DC component)
    ...
    // Compute Mel filterbank center frequencies
    float melLow = hzToMel(lowFreq);
    float melHigh = hzToMel(highFreq);
    float melFilterCenters[numFilters + 2]; // Includes boundaries
    for (size_t i = 0; i < numFilters + 2; i++) {
        melFilterCenters[i] = melToHz(melLow + i * (melHigh - melLow) / (numFilters + 1));
    }

    // Create filterbank matrix
    float filterBank[numFilters][fftBins] = {0}; // Initialize to 0
    for (size_t filter = 1; filter <= numFilters; filter++) {
        for (size_t bin = 0; bin < fftBins; bin++) {
            float freq = bin * samplingFreq / fftSize;
            if (freq < melFilterCenters[filter - 1]) {
                filterBank[filter - 1][bin] = 0;
            } else if (freq <= melFilterCenters[filter]) {
                filterBank[filter - 1][bin] = (freq - melFilterCenters[filter - 1]) /
                                              (melFilterCenters[filter] - melFilterCenters[filter - 1]);
            } else if (freq <= melFilterCenters[filter + 1]) {
                filterBank[filter - 1][bin] = (melFilterCenters[filter + 1] - freq) /
                                              (melFilterCenters[filter + 1] - melFilterCenters[filter]);
            } else {
                filterBank[filter - 1][bin] = 0;
            }
        }
    ...
    float melSpectrum[numFilters] = {0};
    for (size_t filter = 0; filter < numFilters; filter++) {
        for (size_t bin = 0; bin < fftBins; bin++) {
            melSpectrum[filter] += filterBank[filter][bin] * data[bin]; // Accumulate weighted power
        }
        // Ensure no negative or zero values before log
        melSpectrum[filter] = log(fmax(melSpectrum[filter], 1e-6)); // Log-Mel spectrum
    }

    // Compute Discrete Cosine Transform (DCT) for MFCCs
    for (size_t m = 0; m < numMFCCs; m++) {
        mfccOutput[m] = 0; // Initialize
        for (size_t filter = 0; filter < numFilters; filter++) {
            mfccOutput[m] += melSpectrum[filter] *
                             cos(m * PI * (filter + 0.5) / numFilters);
        }
    }
}
